...
static int32_t get_spispeed(void)
{
// 	#define	 CH341A_STM_I2C_20K		0x00
// 	#define	 CH341A_STM_I2C_100K		0x01
// 	#define	 CH341A_STM_I2C_400K		0x02
// 	#define	 CH341A_STM_I2C_750K		0x03
	...
	int speed = CH341A_STM_I2C_100K;
	uint32_t f_spi_req = 0;
	...
	if (spispeed && strlen(spispeed)) {
		...
		if (errno != 0 || spispeed == f_spi_suffix) {
			...
			free(spispeed);
			return CH341A_STM_I2C_100K;
		}
		if (strlen(f_spi_suffix) == 1) {
			if (!strcasecmp(f_spi_suffix, "M")) {
				msg_perr("Error: wrong 'spispeed' parameter, using 100kHz.\n");
				free(spispeed);
				return CH341A_STM_I2C_100K;
			}
				...
				msg_perr("Error: Garbage following 'spispeed' value, using 100kHz.\n");
				free(spispeed);
				return CH341A_STM_I2C_100K;
			}
	...
	switch (f_spi_req) {
		case 20000:  speed = CH341A_STM_I2C_20K; break;
		case 100000: speed = CH341A_STM_I2C_100K; break;
		case 400000: speed = CH341A_STM_I2C_400K; break;
		case 750000: speed = CH341A_STM_I2C_750K; break;
		default: msg_perr("Error: Bad 'spispeed' value, using 100kHz.\n"); break;
	}
 ...
 * without a lot of passion here. If that works fine else we will fail on claiming the interface anyway. */
	...
	if ((config_stream(get_spispeed()) < 0) || (enable_pins(true) < 0))
		goto dealloc_transfers;
